<template>
    <div id="dashboard" class="dashboard">
      ...
      @touchend="handleTouchEnd">
        <div class="side-nav-content">
          <div class="side-nav-header">
            <img src="https://whispr.chat/images/default-profile.jpg" alt="Profile" class="profile-image">
            <div class="user-info">
              <span class="username">JohnDoe</span>
              <span class="whisper-count">42 liked whispers</span>
            </div>
          </div>
      ...
      <div class="dashboard-content">
        <div class="tabs">
          <div class="tab" :class="{ active: activeTab === 'new' }" @click="switchTab('new')">New Whispers</div>
          <div class="tab" :class="{ active: activeTab === 'my' }" @click="switchTab('my')">My Whispers</div>
        </div>
        <div id="whispr-posts">
          <div v-for="(post, index) in displayedPosts" :key="index">
            <div class="whispr-post">
              <div class="whispr-post-header">
                <img :src="post.avatar" :alt="post.username" class="whispr-post-avatar">
                <span class="whispr-post-username">{{ post.username }}</span>
              </div>
              <div class="whispr-post-content">{{ post.content }}</div>
              <div class="whispr-post-actions">
                <span class="whispr-post-action" @click="likeWhisper(index)"><i class="fas fa-heart"></i> {{ post.likes }}</span>
                <span class="whispr-post-action" @click="openModal('comment', index)"><i class="fas fa-comment"></i> {{ post.comments }}</span>
                <span class="whispr-post-action" @click="openModal('share', index)"><i class="fas fa-share"></i> {{ post.shares }}</span>
                <span class="whispr-post-action" @click="reWhisper(index)"><i class="fas fa-retweet"></i> {{ post.rewhispers }}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="add-whisper-btn" @click="openModal('addWhisper',1)" ref="modalAddWhisper">
      ...
      <div class="bottom-nav">
        ...
        <a href="#anonymous-chat"><i class="fas fa-user-secret"></i></a>
  ...
  export default {
    data() {
      ...
      touchThreshold: 50, 
        ...
        newWhisprPosts: [
          {
            username: "Alice",
            avatar: "https://whispr.chat/images/alice-avatar.jpg",
            content: "Just had an amazing idea for a new project! Can't wait to share more details soon. #Excited",
            likes: 15,
            comments: 3,
            shares: 2,
            rewhispers: 1
        ...
        myWhisprPosts: [
          {
            ...
            content: "Working on a new feature for Whispr. Stay tuned for updates! #DeveloperLife",
            likes: 30,
            comments: 12,
            shares: 5,
            rewhispers: 2
    ...
    computed: {
      displayedPosts() {
        return this.activeTab === 'new' ? this.newWhisprPosts : this.myWhisprPosts;
      }
    },
    methods: {
      toggleSideNav() {
